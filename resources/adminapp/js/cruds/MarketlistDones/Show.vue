              ...
              <tbody>
                <tr v-for="(item, k) in entry.items" :key="k">
                  ...
                  <td v-if="entry.status === 'user_acc' || entry.status === 'selesai'">
                      <input v-if="item.approved_qty === null" class="form-control wrapText" type="number" :value="item.approved_qty" @input="updateItemApprovedQty(k, $event)"/>
                      <input v-else disabled class="form-control wrapText" type="number" :value="item.approved_qty" @input="updateItemApprovedQty(k, $event)"/>
                  </td>
                  <td v-if="entry.status === 'user_acc' || entry.status === 'selesai'">
                    <input v-if="item.date === null" class="form-control wrapText" type="date" :value="item.approved_date_front_end" @input="updateItemApprovedDate(k, $event)"/>
                    <input disabled v-else class="form-control wrapText" type="text" :value="item.approved_date"/>
                  </td>
                </tr>
              </tbody>
            </table>
...
export default {
  ...
  computed: {
    ...mapGetters('MarketlistDonesSingle', ['entry', 'lists']),
  },
  watch: {
    '$route.params.id': {
      immediate: true,
  ...
  methods: {
    ...mapActions('MarketlistDonesSingle', ['fetchShowData', 'loading', 'resetState', 'approveData', 'updateData', 'setItemId', 'updateMergedData',
      'rejectData',
      'setItemRequiredDate',
      'setItemNotes',
      'setItemQty',
    ...
    updateItemApprovedDate(index, event, val) {
      val = event.target.value
